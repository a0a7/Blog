...
const _PenguinsMp4 = "/_app/immutable/assets/penguins.b7dc7505.mp4";
const metadata = {
...
<ul><li><p>This template was built with <code>@sveltejs/kit@1.0.0</code></p></li>
...
<p>The <code>preview</code> property is optional, in case you want to customize the preview text. If not provided, the first paragaph of your post will be used instead.</p>
<h2 id="rendering-posts"><a href="#rendering-posts">Rendering Posts</a></h2>
<p>Each individual post is rendered at <code>src/routes/post/[slug]</code>. You’ll notice the route has 3 files:</p>
<pre class="language-undefined"><!-- HTML_TAG_START -->${`<code class="language-undefined">-| +page.js
-| +page.server.js
-| +page.svelte</code>`}<!-- HTML_TAG_END --></pre>
<p>The metadata for the post is loaded in <code>+page.server.js</code>. It is then used in <code>+page.js</code> to import the post’s markdown file. After that, both the post and its metadata are passed into <code>+page.svelte</code> to be rendered.</p>
...
<p><img src="https://og-image.vercel.app/**Getting%20Started**?theme=light&md=1&fontSize=100px&images=https://assets.vercel.com/image/upload/front/assets/design/hyper-color-logo.svg" alt="open-graph"></p>
...
<p>Your posts can be retrieved via <code>import { posts } from &#39;$lib/data/posts&#39;</code>. They are automatically sorted from newest to oldest and contain the metadata for each post.</p>
...
<p>If you wish to render an entire post, you will need to import the <code>.md</code> file directly (as done in <code>src/routes/post/[slug]/+page.js</code>).</p>
...
<p><video${add_attribute("src", _PenguinsMp4, 0)} autoplay muted playsinline loop title="penguins"></video></p>
